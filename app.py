...
class EdgeDMEngine:
    ...
    def find_attack_range_ft(self, player_id: str, action_text: str) -> int:
        """
        Looks up the range (in feet) of whichever spell/attack the action
        text names, from the caster's own character sheet (a 'range_ft'
        field on each prepared spell/attack entry). Falls back to standard
        5e defaults if not found: 5ft for anything that sounds like a melee
        attack, 60ft as a generic default for anything else (e.g. spells
        without an explicit range set).
        """
        caster = self.characters.get(player_id, {})
        for spell in caster.get("preparedSpells", []):
            spell_name = spell.get("name", "")
            if spell_name and spell_name.lower() in action_text.lower():
                if "range_ft" in spell:
                    try:
                        return int(spell["range_ft"])
                    except (ValueError, TypeError):
                        pass

        melee_words = ["punch", "stab", "swing", "slash", "strike", "hit",
                        "unarmed", "sword", "dagger", "fist", "melee"]
        if any(w in action_text.lower() for w in melee_words):
            return 5
        return 60  # generic ranged default when no explicit range_ft is set

    def update_board_positions(self, positions: dict):
        """Stores grid positions received from aruco_tracker.py. Each key is
        an entity id (player_id or enemy id), value is {"x": col, "y": row}
        on the 8x8 grid."""
        self.ledger["board_positions"] = positions
        self.save_state()

    def get_distance_ft(self, entity_a: str, entity_b: str) -> int | None:
        """Returns the straight-line distance in feet between two tracked
        entities on the grid (5ft per square, standard D&D scale), or None
        if either entity hasn't been seen by the camera yet."""
        positions = self.ledger.get("board_positions", {})
        a = positions.get(entity_a)
        b = positions.get(entity_b)
        if not a or not b:
            return None
        dx = a["x"] - b["x"]
        dy = a["y"] - b["y"]
        squares = (dx ** 2 + dy ** 2) ** 0.5
        return round(squares * 5)  # 5 ft per square

    def check_attack_range(self, player_id: str, target_id: str, action_text: str) -> dict | None:
        """
        Checks whether the target is within range of the named attack/spell,
        using real camera-tracked grid positions. Returns None if position
        data isn't available yet (camera not running / entities not seen),
        in which case combat proceeds without a range check — this keeps
        the system fully backward-compatible with sessions run without the
        camera. Returns a dict with in_range/distance_ft/attack_range_ft
        when position data IS available.
        """
        distance_ft = self.get_distance_ft(player_id, target_id)
        if distance_ft is None:
            return None

        attack_range_ft = self.find_attack_range_ft(player_id, action_text)
        return {
            "distance_ft": distance_ft,
            "attack_range_ft": attack_range_ft,
            "in_range": distance_ft <= attack_range_ft,
        }
...
DM:"""
    ...
    def build_master_prompt(self, player_id: str, action_text: str, intent_name: str, trigger_word: str) -> tuple[str, str, dict | None, bool]:
        ...
        if intent_name == "trigger_combat_math":
            ...
            # If the ArUco camera has tracked both the caster and the target,
            # check range BEFORE resolving damage — an out-of-range attack
            # never rolls damage. If either hasn't been seen by the camera
            # yet (range_check is None), combat proceeds as before, fully
            # backward-compatible with sessions run without the camera.
            range_check = self.check_attack_range(player_id, target_id, action_text)

            if range_check is not None and not range_check["in_range"]:
                combat_result = {
                    "target_id": target_id,
                    "in_range": False,
                    "distance_ft": range_check["distance_ft"],
                    "attack_range_ft": range_check["attack_range_ft"],
                    "damage": 0,
                    "target_found": True,
                }
                system_instruction = (
                    f"The player's target is {range_check['distance_ft']} feet away, but "
                    f"their attack only has a range of {range_check['attack_range_ft']} feet. "
                ...
                if range_check is not None:
                    ...
                    combat_result["attack_range_ft"] = range_check["attack_range_ft"]
